...
static void show_dir(lv_obj_t *obj, const char *path)
{
    ...
    //LV_LOG_USER("打开目录 %d", res);
    if (res != LV_FS_RES_OK)
    {
        //LV_LOG_USER("打开目录错误 %d!", res);
        return;
    }
    ...
    while (1)
    {
        ...
        //LV_LOG_USER("lv_fs_dir_read: %d", res);
        if (res != LV_FS_RES_OK)
        {
            //LV_LOG_USER("驱动器、文件或目录不存在 %d!", res);
            break;
        }
    ...
    // 将最多dst_size-1（非空）个字符从src复制到dst。总是添加一个空终止符
