...
/*如果没有跳过lv_conf.h，则包括它*/
#ifndef LV_CONF_SKIP
    #ifdef LV_CONF_PATH                           /*如果有为lv_conf.h定义的路径，请使用它*/
        #define __LV_TO_STR_AUX(x) #x
        #define __LV_TO_STR(x) __LV_TO_STR_AUX(x)
        #include __LV_TO_STR(LV_CONF_PATH)
        #undef __LV_TO_STR_AUX
        #undef __LV_TO_STR
    #elif defined(LV_CONF_INCLUDE_SIMPLE)         /*或者简单地包括lv_conf.h已启用*/
        #include "lv_conf.h"
    #else
        #include "../../lv_conf.h"                /*否则，假设lv_conf.h位于lvgl文件夹旁边*/
    #endif
