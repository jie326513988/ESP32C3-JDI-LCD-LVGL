 ...
 * 这些函数允许您创建模拟世界。
 *
 * 可以将刚体和关节约束添加到世界并运行模拟。您可以获取联系信息以获取接触点、法线以及事件。
 * 您可以查询世界，检查重叠和投射光线或形状。
 * 还有调试信息，如调试绘图、定时信息和计数器。
 * 您可以在此处找到文档：https://box2d.org/
 * @{
 */

/// 为刚体模拟创建一个世界。世界包含实体、形状和约束。
/// 你可创造多达128个世界。每个世界都是完全独立的，可以并行模拟。
///	@return the world id.
...
/// 世界id验证。为多达64K的分配提供验证。
B2_API bool b2World_IsValid( b2WorldId id );

/// 模拟一个世界一步。这将执行碰撞检测、集成和约束求解。
/// @param worldId 要模拟的世界
/// @param timeStep 模拟的时间量，这应该是一个固定的数字。通常为1/60。
/// @param subStepCount 子步骤的数量，增加子步骤计数可以提高精度。通常为4。
...
///根据定义创建刚体。未保留对该定义的引用。因此，您可以在堆栈上创建定义并将其作为指针传递。
///	@code{.c}
...
/// 创建多边形形状并将其附着到实体上。形状定义和几何图形已完全克隆。
/// 直到下一个时间步才会创建联系。
///	@return 访问形状的形状id
